rankhospital <- function(state, outcome, num="best") {
  ...
  if(num=="best"){
    ...
    if(outcome=="heart attack"){
      dat2$Hospital.Name[order(as.numeric(dat2$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack),dat2$Hospital.Name,na.last=T)][num]
    } else if(outcome=="heart failure"){
      dat2$Hospital.Name[order(as.numeric(dat2$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure),dat2$Hospital.Name,na.last=T)][num]
    } else {
      dat2$Hospital.Name[order(as.numeric(dat2$Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia),dat2$Hospital.Name,na.last=T)][num]
    }
  }
}